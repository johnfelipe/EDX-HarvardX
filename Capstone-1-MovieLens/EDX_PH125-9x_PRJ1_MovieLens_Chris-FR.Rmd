...
For this first project of the EDX PH125.9x capstone course, we will be creating a movie recommendation system using the MovieLens dataset. We will use the **10M records** version of this dataset. As a large number of students, one of the issue of this project was the performance of my laptop (2003, 4Gb RAM) to process the `edx` dataset.  
...
```{r edx_load, echo=FALSE, message=FALSE, cache=TRUE}
#######################ORIGINAL##############################
#############################################################
# 
...
```{r create_rmse_results, cache=TRUE}
...
```{r plot_ratings, echo=FALSE, message=FALSE, cache=TRUE,fig.align='center', fig.height=3}
##### PLOT RATING 
...
```{r summary_ratings, echo=FALSE, cache=TRUE}
# rating summary
summary(edx$rating)
```
...
```{r plot_users, cache=TRUE, message=FALSE, echo=FALSE, fig.align='center', fig.height=4, fig.width=8}
...
```{r summary_users, echo=FALSE, cache=TRUE}
# rating summary
...
```{r plot_movies, cache=TRUE, echo=FALSE, fig.align='center', fig.height=4, fig.width=8}
# Load the gridextra package
...
```{r summary_movies, echo=FALSE, cache=TRUE}
# rating summary
...
```{r plot_movies_meanvsnb, cache=TRUE, echo=FALSE, fig.align='center', fig.height=4, fig.width=8}
...
```{r plot_genres_movies, cache=TRUE, echo=FALSE, fig.align='center', fig.height=4}
...
```{r plot_genres, cache=TRUE, echo=FALSE, fig.align='center', fig.height=4, fig.width=8}
...
Microsoft provides a free trial access to test the [Azure ML studio](https://studio.azureml.net/) (100 modules by experiment, 10 Gb storage, 1 hour per experiment, single node execution).  
Azure ML has a recommender module : the [Matchbox Recommender](https://www.microsoft.com/en-us/research/publication/matchbox-large-scale-bayesian-recommendations/).
  
Here is the schema of the full experiment I developed:  

<p>
```{r azureml_global, echo=FALSE, fig.cap="Complete program diagram"}
knitr::include_graphics("figure_ml/azureml_global.PNG")
```
</p>
#### Loading the data
<p>
```{r azureml_load, echo=FALSE, fig.cap="Loading the data"}
knitr::include_graphics("figure_ml/azureml_load.PNG")
```
</p>
...
- the R script module has only **one output**: to overcome this issue, I added a flag (train) and row binded the 2 datasets (edx and validation). We then used a **Split module** to split the `edx` and `valìdation` datasets using the **train** flag as split condition.  

At the end of the R script, validation and edx datasets are concatenated, with an extra train column:  

```{r azureml_codeimport, eval=FALSE}
...
...
<p>
```{r azureml_endimport, echo=FALSE, fig.cap="Loading the data"}
knitr::include_graphics("figure_ml/azureml_endimport.PNG")
```
</p>
...
```{r azureml_code_transformgenre, eval=FALSE}
...
<p>
```{r azureml_genres, echo=FALSE, fig.cap="create the Genres dataset (item features)"}
knitr::include_graphics("figure_ml/azureml_genres.PNG")
```
</p>
...
```{r azureml_code_transformrating, eval=FALSE}
# Map 1-based optional input ports to variables
...
<p>
```{r azureml_transform, echo=FALSE, fig.cap="2 Models"}
knitr::include_graphics("figure_ml/azureml_transform.PNG")
```
</p>
...
```{r model3_10f, cache=TRUE, echo=TRUE, message=FALSE,warning=FALSE}
...
```{r model3_best, cache=TRUE, echo=FALSE}
...
```{r model3_computefullmodel, cache=TRUE, echo=TRUE}
mu <- mean(edx$rating)
...
```{r cluster_colsums, cache=TRUE, echo=TRUE}
...
```{r cluster_dist, cache=TRUE, echo=TRUE}
...
```{r cluster_cutree, cache=TRUE, echo=TRUE}
...
```{r cluster_viewmean, cache=TRUE, echo=TRUE}
...
```{r cluster_addmovieId, cache=TRUE, echo=TRUE}
...
```{r model4_computebestlg, cache=TRUE, include=FALSE, MESSAGE=FALSE, WARNING=FALSE}
# to save the results
df.result <- data.frame( "k" = integer(0), 
                         "lg" = numeric(0),
                         "RMSE" = numeric(0))
...
```{r model4_lg, cache=TRUE, echo=FALSE}
...
```{r model4_plot, cache=TRUE, message=FALSE, echo=FALSE, fig.align='center', fig.height=3}
...
```{r model4_computemodel, cache=TRUE, echo=TRUE}
...
<p>
```{r azureml_score, echo=FALSE, fig.cap="2 Models"}
knitr::include_graphics("figure_ml/azureml_score.PNG")
```
</p>

The RMSE is available in the output dataset.

```{r azureml_code_myrmse, eval=FALSE}
# Map 1-based optional input ports to variables
...
<p>
```{r azureml_rmse, echo=FALSE, fig.cap="2 Models"}
knitr::include_graphics("figure_ml/azureml_model2.PNG")
```
</p>

```{r azureml_results, cache=TRUE, echo=FALSE}
...
```{r model3_validation, cache=TRUE, echo=TRUE}
predicted_ratings <- 
    validation %>% 
    left_join(b_i, by = "movieId") %>%
    left_join(b_u, by = "userId") %>%
...
```{r model4_validation, cache=TRUE, echo=TRUE}
predicted_ratings <- 
    validation %>% 
    left_join(b_i, by = "movieId") %>%
    left_join(b_u, by = "userId") %>%
...
```{r models_conclusion, cache=TRUE, echo=FALSE}
rmse_results %>% knitr::kable()
```

As expected from the average of the cross validations, the model with the best result is the 'Regularized Movie + User Effect + Genre effect Model' with an RMSE of **`r round(rmse_model4, 4)`**. This value is near the ones computed using 5-folds cross validation on the `edx` dataset: average= `r round(df.model4$meanRMSE, 4)` (min= `r round(df.model4$minRMSE, 4)`, max= `r round(df.model4$maxRMSE, 4)`).  
